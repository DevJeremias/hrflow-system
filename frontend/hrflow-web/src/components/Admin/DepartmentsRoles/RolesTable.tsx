...
const RolesTable: React.FC<Props> = ({ roles, onEdit, onDelete }) => {
  const getLevelClass = (level: string) => {
    const classes: any = {
      ...
      'Sênior': 'bg-orange-50 text-orange-600',
            ...
            <tr key={role.id} className="hover:bg-slate-50/50 transition-colors group">
              <td className="py-4 px-6">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-orange-50 text-primary rounded-lg group-hover:bg-primary group-hover:text-white transition-all">
                    <Briefcase size={16} />
                  </div>
